...
async function main(day, limit = 100, offset = 0) {
    ...
    preEntries = generatePathVariable(preEntries, [/* "path_1", */ "path_1", "path_2"]);
...
// Helper Functions
const getUtmAndCallToActionKey = ({ utm, callToAction }) => (`${utm.campaignName}|${callToAction.actionTypeId ?? ''}|${callToAction.storeReferenceId ?? ''}|${(callToAction.storeReferenceIds || []).sort((a, b) => a - b).join(',')}|${callToAction.macroId ?? ''}|${callToAction.brandId ?? ''}`);
...
const getPathFromPreEntry = ({ utm, shortLink }) => {
    ...
    const queryParams = `utm_source=${utm.campaignSource || ''}&utm_medium=${utm.campaignMedium || ''}&utm_content=${utm.campaignContent || ''}&utm_campaign=${utm.campaignName}&utm_term=${utm.campaignTerm || ''}`;
...
const generateCallToActionShortLinks = async (preEntries) => {
    ...
    return preEntries.map(preEntry => {
        ...
        return {
            ...
            shortLinks: callToActions.map(callToAction => shortLinkMap.get(getUtmAndCallToActionKey({ utm, callToAction }))),
        };
    });
};
const xxxx = async (preMap) => {
    const integration = new LbApiOperacionesIntegration();
    const responses = await integration.createAllShortLink(Array.from(preMap.entries()).map(([key, value]) => ({
        key,
        value
    })));
...
const generatePreEntries = (storesMap) => {
    ...
    for (const data of Array.from(storesMap.values())) {
        ...
        const { variables, storeReferenceIds } = generateVariablesAndStoreReferenceIds(campaign.variables, { store, skus }) ?? {
            variables: undefined,
            storeReferenceIds: undefined
        };
        if (!variables || !storeReferenceIds)
            continue;
        ...
        const callToActions = generateCallToActionPaths(["path_1", "path_2"], storeReferenceIds);
        if (!callToActions)
            continue;
...
const getUtm = (day, status, locationId, name) => {
    ...
    const campaign = `${getCityId(locationId)}_${getProvider(locationId)}_${asset}_${payer}_${formatMMMDD(term)}_${type}_${status}_${name.replace(/[^a-zA-Z0-9]/g, '-')}`;
